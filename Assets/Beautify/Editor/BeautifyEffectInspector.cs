...
namespace BeautifyEffect {
	...
	public class BeautifyEffectInspector : Editor {
		...
		bool expandOutlineSection, expandLUTSection, expandBlurSection, expandAASection;
		...
		SerializedProperty _depthOfFieldDebug, _depthOfFieldAutofocus, _depthofFieldAutofocusViewportPoint, _depthOfFieldAutofocusMinDistance, _depthOfFieldAutofocusMaxDistance, _depthOfFieldAutofocusLayerMask;
		...
		SerializedProperty _depthOfFieldForegroundDistance, _depthOfFieldMaxBrightness, _depthOfFieldMaxDistance, _depthOfFieldBokeh, _depthOfFieldBokehThreshold, _depthOfFieldBokehIntensity;
		...
		public override void OnInspectorGUI () {
			...
			if (_quality.intValue != (int)BEAUTIFY_QUALITY.Basic) {
				...
				if (isAAEnabled) {
					EditorGUILayout.BeginHorizontal ();
					EditorGUILayout.BeginHorizontal (GUILayout.Width (121));
					expandAASection = EditorGUILayout.Foldout (expandAASection, new GUIContent ("AntiAliasing", "Intensity of antialiasing."), sectionHeaderNormalStyle);
					EditorGUILayout.EndHorizontal ();
				...
				if (testFeatureEnabled (isDoFEnabled)) {
					...
					if (expandDoFSection) {
						...
						if (_depthOfFieldAutofocus.boolValue) {
							...
							float dofDistance = _effect.depthOfFieldCurrentFocalPointDistance;
							string dist = Application.isPlaying ? dofDistance.ToString () : "---";
							EditorGUILayout.LabelField (new GUIContent ("      Current Distance"), new GUIContent (dist));
						} else {
